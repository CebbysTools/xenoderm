...
def main() -> None:
    ...
    try:
        ...
        with open_session() as session:
            ...
            logging.info(f"Found Ghidra release '{version}'")
            
            ghidra_home = Path(get_string("GHIDRA_PATH")).resolve()
            if ghidra_home.exists():
                rmtree(ghidra_home)
                logging.info(f"Removed existing Ghidra installation from '{ghidra_home}'")
            ...
            logging.info(f"Successfully installed Ghidra version '{version}'")
...
def validate_environment():
    try:
        errors: list[str] = []
        for key in ["GHIDRA_VERSION", "GHIDRA_PATH", "GITHUB_OUTPUT"]:
            try:
                get_string(key)
            except:
                errors.append(key)
        if errors:
            raise ValueError(f"Missing environment variable(s) '{', '.join(errors)}'")
    except BaseException as e:
        raise BaseException("Failure validating environment") from e
...
def open_session():
    try:
        ...
        token = get_string("GITHUB_TOKEN", default="default")
        if token != "default":
...
def get_all_releases(session: requests.Session) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        out = resp.json()
        logging.info("Query for Ghidra releases found %d releases", len(out))
        try:
...
def find_release(releases: List[Dict[str, Any]], version: str) -> Dict[str, Any]:
    release:Any = None
    try:
        logging.info("Looking for Ghidra release matching version '%s'", version)
        length = len(releases)
        if length == 0:
...
def unzip_asset(asset: Path):
    try:
        parent = asset.parent
        logging.info("Unzipping asset into directory %s", parent.resolve())
    ...
    logging.info("Unzipped asset successfully")
